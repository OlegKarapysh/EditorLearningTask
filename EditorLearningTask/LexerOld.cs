...
public class LexerOld
{
        ...
        for (int lineNum = 0; lineNum < lines.Length; lineNum++)
        {
            ...
            if (lineEndState != 0)
            {
                if (lineEndState == SqlTokenTypes.TOKEN_COMMENT)
                {
                    ...
                    if (endIdx >= 0)
                    {
                        ...
                        tokens.Add(new Token(0, endIdx + 2, SqlTokenTypes.TOKEN_COMMENT, line.Substring(0, endIdx + 2).AsMemory()));
                        pos = endIdx + 2;
                        lineEndState = 0;
                        ...
                        while (pos < length)
                        {
                            ...
                            if (c == '/' && pos + 1 < length && line[pos + 1] == '*')
                            {
                                ...
                                tokens.Add(new Token(pos, length - pos, SqlTokenTypes.TOKEN_COMMENT, line.Substring(pos).AsMemory()));
                                pos = length;
                                break;
                            }
                            if (c == '\'')
                    ...
                    else
                    {
                        ...
                        tokens.Add(new Token(0, length, SqlTokenTypes.TOKEN_COMMENT, line.AsMemory()));
                ...
                else if (lineEndState == SqlTokenTypes.TOKEN_STRING)
                {
                    ...
                    tokens.Add(new Token(0, closed ? start : length, SqlTokenTypes.TOKEN_STRING,
                        ("'" + line.Substring(0, closed ? start : length)).AsMemory()));
                    if (closed)
                    {
                        lineEndState = 0;
            ...
            while (pos < length)
            {
                ...
                if (c == '/' && pos + 1 < length && line[pos + 1] == '*')
                {
                    ...
                    if (endIdx >= 0)
                    {
                        tokens.Add(new Token(pos, endIdx + 2 - pos, SqlTokenTypes.TOKEN_COMMENT, line.Substring(pos, endIdx + 2 - pos).AsMemory()));
                        pos = endIdx + 2;
                        continue;
                    }
                    else
    ...
    private void HandleWhitespace(string line, ref int pos, List<Token> tokens)
    {
        ...
        tokens.Add(new Token(wsStart, pos - wsStart, SqlTokenTypes.TOKEN_WHITESPACE, line.Substring(wsStart, pos - wsStart).AsMemory()));
    }
    ...
    private bool HandleStringLiteral(string line, ref int pos, ref int lineEndState, ref int multilineTokenStart, List<Token> tokens)
    {
        ...
        if (closed)
        {
            tokens.Add(new Token(strStart, sb.Length, SqlTokenTypes.TOKEN_STRING, sb.ToString().AsMemory()));
            return false;
        }
        else
        {
    ...
    private void HandleNumber(string line, ref int pos, List<Token> tokens)
    {
        ...
        tokens.Add(new Token(numStart, pos - numStart, SqlTokenTypes.TOKEN_NUMBER, line.Substring(numStart, pos - numStart).AsMemory()));
    }
    ...
    private void HandleIdentifierOrKeyword(string line, ref int pos, List<Token> tokens)
    {
        ...
        if (kwType != -1)
            tokens.Add(new Token(idStart, pos - idStart, kwType, text.AsMemory()));
        else
            tokens.Add(new Token(idStart, pos - idStart, SqlTokenTypes.TOKEN_IDENTIFIER, text.AsMemory()));
    }

    private void HandleSymbol(string line, ref int pos, List<Token> tokens)
    {
        tokens.Add(new Token(pos, 1, SqlTokenTypes.TOKEN_SYMBOL, line.Substring(pos, 1).AsMemory()));
        pos++;
    }

    private void HandleUnknown(string line, ref int pos, List<Token> tokens)
    {
        tokens.Add(new Token(pos, 1, SqlTokenTypes.TOKEN_UNKNOWN, line.Substring(pos, 1).AsMemory()));
        pos++;
    }
}